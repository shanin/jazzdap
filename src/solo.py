...
class Audio:
    ...
    def resampled_audio(self):
        transform = torchaudio.transforms.Resample(
            orig_freq=self.sample_rate, new_freq=self.resample_rate
        )
...
class SfnmfFeatures(GenericMixinFeatures):
    def _generate_features(self, Fs=22050, hop=256):
        ...
        audio_fpath = "foo"
        input_args = [
            "{0}".format(audio_fpath),
            "--samplingRate={0}".format(Fs),
            "--hopsize={0}".format(self.window),
        ]
...
class OnsetsAndFramesLabeling(GenericMixinLabeling):
    def _generate_labels(self, scaling_const=5.0, onset_windows=2):
        ...
        sec2step = lambda x: np.round(x / self.window)
        onsets = sec2step(self.melody.onset.values)
        offsets = sec2step(self.melody.onset.values + self.melody.duration.values)
        pitches = self.melody.pitch.values
...
class WeimarInit(MixinInit):
    ...
    def _parse_transcription_info(self):
        ...
        query = (
            f"SELECT {colnames_str} FROM transcription_info WHERE melid = {self.melid}"
        )
        transcription_info = pd.DataFrame(
            self._cursor.execute(query).fetchall(), columns=TRANSCRIPTION_INFO_COLNAMES
        )
    ...
    def _parse_track_info(self):
        ...
        track_info = pd.DataFrame(
            self._cursor.execute(query).fetchall(), columns=TRACK_INFO_COLNAMES
        )
    ...
    def _parse_melody(self):
        ...
        self.melody = pd.DataFrame(
            self._cursor.execute(query).fetchall(), columns=MELODY_COLNAMES
        )
    ...
    def _parse_beats(self):
        ...
        self.beats = pd.DataFrame(
            self._cursor.execute(query).fetchall(), columns=BEATS_COLNAMES
        )

    def __init__(
        self, melid, database_filepath, audio_folder=None, features_folder=None
    ):
        ...
        if hasattr(self, "_load_audio"):
            filename = os.path.join(self.audio_folder, f"{self.filename_solo}.wav")
            self._load_audio(filename)

        if hasattr(self, "_load_features"):
            feature_filename = os.path.join(
                self.features_folder, f"{str(self.melid).zfill(3)}.npy"
            )
...
init_classes = {"regular": RegularInit, "weimar": WeimarInit}

raw_data_classes = {"audio": Audio}

feature_classes = {"sfnmf": SfnmfFeatures, "crepe": CrepeFeatures}

labeling_classes = {"crnn": CRNNLabeling, "onsetsandframes": OnsetsAndFramesLabeling}
...
def construct_solo_class(
    dataset_type="regular", feature_type=None, raw_data_type="audio", labeling_type=None
):
    mixins = []
