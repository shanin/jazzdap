    ...
    def __init__(self, dataset, config, tag=None, test_time=False):
        ...
        if self.tag is None:
            ...
            self.cache_folder = os.path.join(
                config["sampler"]["cache_folder"], self.tag
            )
            if os.path.exists(self.cache_folder):
                self.X = torch.load(os.path.join(self.cache_folder, "X.pt"))
                self.y = torch.load(os.path.join(self.cache_folder, "y.pt"))
                self.track_lengths = torch.load(
                    os.path.join(self.cache_folder, "track_lengths.pt")
                )
                self.number_of_samples = torch.load(
                    os.path.join(self.cache_folder, "number_of_samples.pt")
                )
            else:
                self._assemble_tensors()
...
class GenericCRNNSampler(GenericFrameLevelSampler):
    ...
    def _prepare_HF0_tensor(self, features):
        ...
        return (
            torch.tensor(HF0, dtype=torch.float),
            length_of_sequence,
            number_of_samples,
        )
        ...
        if len(labels):
            ...
            y = np.reshape(
                ...
                (
                    ...
                    self.number_of_patches,
                    self.patch_size,
                    self.num_label_channels,
                ),
            )
            return (
                torch.tensor(y, dtype=torch.int).transpose(1, 3).transpose(2, 3),
                length_of_sequence,
                number_of_samples,
            )
        else:
...
class GenericOnsetsAndFramesSampler(GenericFrameLevelSampler):
    ...
    def _prepare_HF0_tensor(self, features):
        ...
        number_of_samples = int(HF0.shape[0] / self.segment_length)
        HF0 = np.reshape(
            HF0, (number_of_samples, 1, self.segment_length, self.feature_size)
        )
        return (
            torch.tensor(HF0, dtype=torch.float),
            length_of_sequence,
    ...
    def _prepare_labels_tensor(self, labels):
        if len(labels):
            ...
            return (
                torch.tensor(y, dtype=torch.float).transpose(1, 3).transpose(2, 3),
                length_of_sequence,
                number_of_samples,
            )
        else:
