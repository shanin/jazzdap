...
def compute_raw_pitch_accuracy_HF0(HF0, labels):
    ...
    stepNotes_class = np.int((numberOfClasses - 2) / (12.0 * np.log2(1760.0 / 55.0)))

    stepNotes_dataset = np.int((HF0.shape[0] - 1) / (12.0 * np.log2(1760.0 / 55.0)))
    numberOfF0 = HF0.shape[0]
    F0labels = 55.0 * (
        2 ** (np.arange(numberOfF0, dtype=np.double) / (12 * stepNotes_dataset))
    )
    ...
    F0labels = 55.0 * (
        2 ** (np.arange(numberOfF0, dtype=np.double) / (12 * stepNotes_class))
    )

    # Non-linear quantization levels
    nlq_levels = F0labels * (2 ** (1.0 / (12 * (stepNotes_class + 1))))
    ...
    raw_pitch_accuracy = np.sum(labels[labels != 0] == y_pred[labels != 0]) / np.float(
        np.sum(labels != 0)
    )
    raw_chroma_accuracy = np.sum(
        (np.abs(labels[labels != 0] - y_pred[labels != 0])) % 12 == 0
    ) / np.float(np.sum(labels != 0))
...
def SIMM(  # the data to be fitted to:
    ...
    HGAMMA[:, lambda_diag > 0] = HGAMMA[:, lambda_diag > 0] / np.outer(
        np.ones(P), lambda_diag[lambda_diag > 0]
    )
    ...
    for n in np.arange(numberOfIterations):
        ...
        HF0 = (
            HF0
            * (np.dot(WF0.T, tempNumFbyN) / np.maximum(np.dot(WF0.T, tempDenFbyN), eps))
            ** omega
        )
        if labels is not None:
            rpa, rca = compute_raw_pitch_accuracy_HF0(HF0, labels)
        ...
        if verbose:
            print(
                "Reconstruction error difference after HF0   : ",
        ...
        if verbose:
            print(
                "Reconstruction error difference after HPHI  : ",
                recoError[counterError] - recoError[counterError - 1],
            )
        counterError += 1
        ...
        HM = np.maximum(
            HM
            * (np.dot(WM.T, tempNumFbyN) / np.maximum(np.dot(WM.T, tempDenFbyN), eps))
            ** omega,
            eps,
        )
        ...
        if verbose:
            print(
                "Reconstruction error difference after HM    : ",
                recoError[counterError] - recoError[counterError - 1],
            )
        counterError += 1
        ...
        HGAMMA = np.maximum(
            HGAMMA
            * (
                np.dot(WGAMMA.T, np.dot(tempNumFbyN, HPHI.T))
                / np.maximum(np.dot(WGAMMA.T, np.dot(tempDenFbyN, HPHI.T)), eps)
            )
            ** omega,
            eps,
        )
        ...
        if verbose:
            print(
                "Reconstruction error difference after HGAMMA: ",
                recoError[counterError] - recoError[counterError - 1],
            )
        ...
        if n > WMstart:  # this test can be used such that WM is updated only
            ...
            WM = np.maximum(
                WM
                * (
                    np.dot(tempNumFbyN, HM.T)
                    / np.maximum(np.dot(tempDenFbyN, HM.T), eps)
                )
                ** omega,
                eps,
            )
            ...
            WM[:, sumWM > 0] = WM[:, sumWM > 0] / np.outer(np.ones(F), sumWM[sumWM > 0])
            ...
            if verbose:
                print(
                    "Reconstruction error difference after WM    : ",
                    recoError[counterError] - recoError[counterError - 1],
                )
            counterError += 1
